...
interface ScansStatusFilterConfig {
  label: string;
  bgColor: string;
}

const statusConfigs: Record<string, ScansStatusFilterConfig> = {
  all: { label: "All Status", bgColor: "bg-muted" },
  active: { label: "Active", bgColor: "bg-yellow-500" },
...
function ScansStatusFilterFallback() {
  return (
    <Button
      variant="outline"
      disabled
...
function ScansStatusFilterInner() {
...
export function ScansStatusFilter() {
  return (
    <Suspense fallback={<ScansStatusFilterFallback />}>
      <ScansStatusFilterInner />
    </Suspense>
  );
}
