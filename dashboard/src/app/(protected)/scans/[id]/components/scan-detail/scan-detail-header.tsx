...
function StatusBadge({ status }: { status: string }) {
  ...
  if (s === "COMPLETED") {
    return (
      <div className="flex items-center gap-1.5 rounded border border-blue-500/20 bg-blue-500/5 px-2.5 py-1">
  ...
  if (s === "FAILED") {
    return (
      <div className="flex items-center gap-1.5 rounded border border-red-500/20 bg-red-500/5 px-2.5 py-1">
        <Circle className="h-1.5 w-1.5 fill-red-400 text-red-400" />
        <span className="text-xs text-red-400">{status}</span>
      </div>
    );
  }

  // DRAFT and any unknown status
  return (
    <div className="flex items-center gap-1.5 rounded border border-zinc-700 bg-zinc-800 px-2.5 py-1">
