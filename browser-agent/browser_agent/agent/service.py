...
if TYPE_CHECKING:
	from browser_agent.skills.views import Skill

from dotenv import load_dotenv

from browser_agent.agent.cloud_events import (
...
from browser_agent.agent.message_manager.utils import save_conversation
from browser_agent.llm.base import BaseChatModel
from browser_agent.llm.exceptions import ModelProviderError, ModelRateLimitError
from browser_agent.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from browser_agent.tokens.service import TokenCost
...
from browser_agent import Browser, BrowserProfile, BrowserSession
from browser_agent.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from browser_agent.agent.gif import create_history_gif
from browser_agent.agent.message_manager.service import (
	MessageManager,
)
from browser_agent.agent.prompts import SystemPrompt
from browser_agent.agent.views import (
	ActionResult,
	AgentError,
	AgentHistory,
	AgentHistoryList,
...
from browser_agent.browser.events import _get_timeout
from browser_agent.browser.session import DEFAULT_BROWSER_PROFILE
from browser_agent.browser.views import BrowserStateSummary
from browser_agent.config import CONFIG
from browser_agent.dom.views import DOMInteractedElement, MatchLevel
from browser_agent.filesystem.file_system import FileSystem
from browser_agent.observability import observe, observe_debug
from browser_agent.telemetry.service import ProductTelemetry
from browser_agent.telemetry.views import AgentTelemetryEvent
from browser_agent.tools.registry.views import ActionModel
from browser_agent.tools.service import Tools
from browser_agent.utils import (
	URL_PATTERN,
	_log_pretty_path,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from browser_agent.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from browser_agent import ChatBrowserUse

				llm = ChatBrowserUse()

		# set flashmode = True if llm is ChatBrowserUse
		if llm.provider == 'browser-agent':
			flash_mode = True
		...
		supports_coordinate_clicking = any(
			pattern in model_name for pattern in ['claude-sonnet-4', 'claude-opus-4', 'gemini-3-pro', 'browser-agent/']
		)
		...
		if skill_service is not None:
			...
			from browser_agent.skills import SkillService
		...
		# Check if model is a browser-agent fine-tuned model (uses simplified prompts)
		is_browser_use_model = 'browser-agent/' in self.llm.model.lower()
		...
		# wal_path = CONFIG.BROWSER_AGENT_CONFIG_DIR / 'events' / f'{self.session_id}.jsonl'
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'browser_agent.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from browser_agent.screenshots.service import ScreenshotService
	...
	def _set_browser_use_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the browser-agent package"""
	...
	async def _judge_trace(self) -> JudgementResult | None:
		...
		if self.judge_llm.provider == 'browser-agent':
		...
		if self.history.history[-1].result[-1].is_done:
			...
			if judgement:
				...
				if judgement.reached_captcha:
					...
					judge_log += '   👉 🥷 Use Browser Use Cloud for the most stealth browser infra: https://docs.browser-agent.com/customize/browser/remote\n'
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from browser_agent.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		if CONFIG.BROWSER_AGENT_VERSION_CHECK:
			latest_version = await check_latest_browser_use_version()
			if latest_version and latest_version != self.version:
				self.logger.info(
					f'📦 Newer version available: {latest_version} (current: {self.version}). Upgrade with: uv add browser-agent=={latest_version}'
				)
	...
	def _log_first_step_startup(self) -> None:
		...
		if len(self.history.history) == 0:
			self.logger.info(
				f'Starting a browser-agent agent with version {self.version}, with provider={self.llm.provider} and model={self.llm.model}'
			)
	...
	def _log_final_outcome_messages(self) -> None:
		...
		if is_successful is False or is_successful is None:
			...
			self.logger.info('   Open a short issue on GitHub: https://github.com/browser-agent/browser-agent/issues')
	...
	async def run(
		...
		from browser_agent.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from browser_agent.agent.views import RerunSummaryAction
		...
		from browser_agent.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
	...
	async def _execute_ai_step(
		...
		"""
		from browser_agent.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from browser_agent.llm.messages import SystemMessage, UserMessage
		from browser_agent.utils import sanitize_surrogates
		...
		try:
			from browser_agent.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from browser_agent.agent.variable_detector import detect_variables_in_history
