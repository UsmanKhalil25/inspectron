...
Browser Actor is a web automation library built on CDP (Chrome DevTools Protocol) that provides low-level browser automation capabilities within the browser-agent ecosystem.

## Usage

### Integrated with Browser (Recommended)
```python
from browser_agent import Browser  # Alias for BrowserSession
...
```python
from browser_agent.actor import Page, Element, Mouse
...
**This is browser-agent actor, NOT Playwright or Selenium.** Only use the methods documented above.
