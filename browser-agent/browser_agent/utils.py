...
def get_browser_use_version() -> str:
	"""Get the browser-agent package version using the same logic as Agent._set_browser_use_version_and_source"""
	try:
		...
		version = str(get_version('browser-agent'))
		os.environ['LIBRARY_VERSION'] = version
		return version

	except Exception as e:
		logger.debug(f'Error detecting browser-agent version: {type(e).__name__}: {e}')
		return 'unknown'


async def check_latest_browser_use_version() -> str | None:
	"""Check the latest version of browser-agent from PyPI asynchronously.
	...
	try:
		async with httpx.AsyncClient(timeout=3.0) as client:
			response = await client.get('https://pypi.org/pypi/browser-agent/json')
