...
from browser_agent.browser.cloud.views import CloudBrowserParams
from browser_agent.config import CONFIG
from browser_agent.utils import _log_pretty_path, logger


def _get_enable_default_extensions_default() -> bool:
	"""Get the default value for enable_default_extensions from env var or True."""
	env_val = os.getenv('BROWSER_AGENT_DISABLE_EXTENSIONS')
...
# Using constants from central location in browser_agent.config
...
class BrowserLaunchArgs(BaseModel):
	...
	def set_default_downloads_path(self) -> Self:
		...
		if self.downloads_path is None:
			...
			downloads_path = Path(tempfile.gettempdir()) / f'browser-agent-downloads-{unique_id}'
...
class BrowserLaunchPersistentContextArgs(BrowserLaunchArgs, BrowserContextArgs):
	...
	def validate_user_data_dir(cls, v: str | Path | None) -> str | Path:
		...
		if v is None:
			return tempfile.mkdtemp(prefix='browser-agent-user-data-dir-')
...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	use_cloud: bool = Field(
		default=False,
		description='Use browser-agent cloud browser service instead of local browser',
	)
	...
	def warn_user_data_dir_non_default_version(self) -> Self:
		...
		if self.user_data_dir == CONFIG.BROWSER_AGENT_DEFAULT_USER_DATA_DIR and is_not_using_default_chromium:
			alternate_name = (
			...
			self.user_data_dir = CONFIG.BROWSER_AGENT_DEFAULT_USER_DATA_DIR.parent / f'default-{alternate_name}'
		return self
	...
	def _copy_profile(self) -> None:
		...
		if 'browser-agent-user-data-dir-' in user_data_str.lower():
		...
		temp_dir = tempfile.mkdtemp(prefix='browser-agent-user-data-dir-')
	...
	def _ensure_default_extensions_downloaded(self) -> list[str]:
		...
		cache_dir = CONFIG.BROWSER_AGENT_EXTENSIONS_DIR
