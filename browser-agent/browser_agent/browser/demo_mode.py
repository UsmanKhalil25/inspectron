...
_DEMO_PANEL_SCRIPT = r"""(function () {
  ...
  const SESSION_ID = '__BROWSER_AGENT_SESSION_ID_PLACEHOLDER__';
  const EXCLUDE_ATTR = 'data-browser-agent-exclude-' + SESSION_ID;
  const PANEL_ID = 'browser-agent-demo-panel';
  const STYLE_ID = 'browser-agent-demo-panel-style';
  ...
  const TOGGLE_BUTTON_ID = 'browser-agent-demo-toggle';
  ...
  function initializePanel() {
    ...
    if (savedWidth) {
      document.documentElement.style.setProperty('--browser-agent-demo-panel-width', `${savedWidth}px`);
    }
  ...
  function addStyles() {
    ...
    style.textContent = `
      #${PANEL_ID} {
        ...
        top: 0;
        right: 0;
        width: var(--browser-agent-demo-panel-width, 340px);
      ...
      #${PANEL_ID} .browser-agent-demo-header {
      ...
      #${PANEL_ID} .browser-agent-demo-header h1 {
        font-size: 15px;
      ...
      #${PANEL_ID} .browser-agent-logo img {
        height: 36px;
      }

      #${PANEL_ID} .browser-agent-header-actions {
        margin-left: auto;
        display: flex;
        align-items: center;
        gap: 8px;
      ...
      #${PANEL_ID} .browser-agent-close-btn {
        width: 28px;
        height: 28px;
        border-radius: 50%;
      ...
      #${PANEL_ID} .browser-agent-close-btn:hover {
        background: rgba(255, 255, 255, 0.15);
        border-color: rgba(255, 255, 255, 0.35);
      }

      #${PANEL_ID} .browser-agent-demo-body {
        flex: 1;
        overflow-y: auto;
      ...
      #${PANEL_ID} .browser-agent-demo-body::-webkit-scrollbar {
        width: 8px;
      }

      #${PANEL_ID} .browser-agent-demo-body::-webkit-scrollbar-thumb {
        background: rgba(255, 255, 255, 0.25);
        border-radius: 999px;
      }

      .browser-agent-demo-entry {
        display: flex;
        gap: 12px;
        padding: 10px 18px;
        border-left: 2px solid transparent;
        border-bottom: 1px solid rgba(255, 255, 255, 0.04);
        animation: browser-agent-fade-in 0.25s ease;
        background: #000000;
      }

      .browser-agent-demo-entry:last-child {
        border-bottom-color: transparent;
      }

      .browser-agent-entry-icon {
        font-size: 16px;
        line-height: 1.2;
        width: 20px;
      }

      .browser-agent-entry-content {
        flex: 1;
        min-width: 0;
      }

      .browser-agent-entry-meta {
        font-size: 11px;
        letter-spacing: 0.08em;
        text-transform: uppercase;
        color: white;
      ...
      .browser-agent-markdown-content {
        margin: 0;
        line-height: 1.5;
      }

      .browser-agent-markdown-content p {
        margin: 0 0 8px 0;
      }

      .browser-agent-markdown-content p:last-child {
        margin-bottom: 0;
      }

      .browser-agent-markdown-content h1,
      .browser-agent-markdown-content h2,
      .browser-agent-markdown-content h3 {
        margin: 8px 0 4px 0;
        font-weight: 600;
        color: #f8f9ff;
      }
      ...
      .browser-agent-markdown-content code {
      ...
      .browser-agent-markdown-content pre {
      ...
      .browser-agent-markdown-content pre code {
      ...
      .browser-agent-markdown-content ul,
      .browser-agent-markdown-content ol {
        margin: 4px 0 4px 16px;
        padding: 0;
      }

      .browser-agent-markdown-content li {
        margin: 2px 0;
      }

      .browser-agent-markdown-content a {
        color: #60a5fa;
        text-decoration: underline;
      }

      .browser-agent-markdown-content a:hover {
        color: #93c5fd;
      }

      .browser-agent-markdown-content strong {
        font-weight: 600;
        color: #f8f9ff;
      }

      .browser-agent-markdown-content em {
        font-style: italic;
      }

      .browser-agent-demo-entry:not(.expanded) .browser-agent-markdown-content {
        max-height: 120px;
        overflow: hidden;
        mask-image: linear-gradient(to bottom, rgba(0,0,0,1), rgba(0,0,0,0));
      }

      .browser-agent-entry-toggle {
      ...
      .browser-agent-demo-entry.level-info { border-left-color: #60a5fa; }
      .browser-agent-demo-entry.level-action { border-left-color: #34d399; }
      .browser-agent-demo-entry.level-thought { border-left-color: #f97316; }
      .browser-agent-demo-entry.level-warning { border-left-color: #fbbf24; }
      .browser-agent-demo-entry.level-success { border-left-color: #22c55e; }
      .browser-agent-demo-entry.level-error { border-left-color: #f87171; }

      @keyframes browser-agent-fade-in {
  ...
  function buildPanel() {
    ...
    header.className = 'browser-agent-demo-header';
    const title = document.createElement('div');
    title.className = 'browser-agent-logo';
    const logo = document.createElement('img');
    logo.src = 'https://raw.githubusercontent.com/browser-agent/browser-agent/main/static/browser-agent-dark.png';
    logo.alt = 'Browser-use';
    logo.loading = 'lazy';
    title.appendChild(logo);
    const actions = document.createElement('div');
    actions.className = 'browser-agent-header-actions';
    const closeBtn = document.createElement('button');
    closeBtn.type = 'button';
    closeBtn.className = 'browser-agent-close-btn';
    ...
    body.className = 'browser-agent-demo-body';
  ...
  function adjustLayout() {
    ...
    document.documentElement.style.setProperty('--browser-agent-demo-panel-width', `${width}px`);
    if (state.isOpen) {
  ...
  function hydrateFromStoredMarkup() {
    ...
    try {
      ...
      if (html) {
        ...
        for (const entryNode of state.list.querySelectorAll('.browser-agent-demo-entry')) {
          const toggle = entryNode.querySelector('.browser-agent-entry-toggle');
          if (toggle) {
  ...
  function createEntryNode(entry) {
    ...
    row.className = `browser-agent-demo-entry level-${entry.level}`;
    row.setAttribute('data-id', entry.id);

    const icon = document.createElement('span');
    icon.className = 'browser-agent-entry-icon';
    icon.textContent = LEVEL_ICONS[entry.level] || LEVEL_ICONS.info;

    const content = document.createElement('div');
    content.className = 'browser-agent-entry-content';

    const meta = document.createElement('div');
    meta.className = 'browser-agent-entry-meta';
    const time = formatTime(entry.timestamp);
    const label = LEVEL_LABELS[entry.level] || entry.level;
    meta.innerHTML = `<span>${time}</span><span>${label}</span>`;

    const messageWrapper = document.createElement('div');
    messageWrapper.className = 'browser-agent-entry-message';
    const messageText = entry.message.trim();
    const messageHtml = messageText;
    const message = document.createElement('div');
    message.className = 'browser-agent-markdown-content';
    ...
    if (messageText.length > 160) {
      ...
      toggle.type = 'button';
      toggle.className = 'browser-agent-entry-toggle';
  ...
  function applyPersistedExpansion(node) {
    ...
    try {
      ...
      if (id && expanded.has(id)) {
        ...
        const toggle = node.querySelector('.browser-agent-entry-toggle');
        if (toggle) {
  ...
  window.addEventListener('browser-agent-log', handleLogEvent);
})();
"""
...
class DemoMode:
	...
	def _load_script(self) -> str:
		...
		script_with_session_id = self._script_source.replace('__BROWSER_AGENT_SESSION_ID_PLACEHOLDER__', session_id)
...
(() => {{
	...
	const event = new CustomEvent('browser-agent-log', {{ detail }});
