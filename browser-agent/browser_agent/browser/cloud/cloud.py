"""Cloud browser service integration for browser-agent.

This module provides integration with the browser-agent cloud browser service.
...
from browser_agent.browser.cloud.views import CloudBrowserAuthError, CloudBrowserError, CloudBrowserResponse, CreateBrowserRequest
from browser_agent.sync.auth import CloudAuthConfig

logger = logging.getLogger(__name__)


class CloudBrowserClient:
	"""Client for browser-agent cloud browser service."""

	def __init__(self, api_base_url: str = 'https://api.browser-agent.com'):
	...
	async def create_browser(
		...
		"""Create a new cloud browser instance. For full docs refer to https://docs.cloud.browser-agent.com/api-reference/v-2-api-current/browsers/create-browser-session-browsers-post
		...
		if not api_token:
			raise CloudBrowserAuthError(
				'No authentication token found. Please set browser_use_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.browser-agent.com/new-api-key'
			)

		headers = {'X-browser-agent-API-Key': api_token, 'Content-Type': 'application/json', **(extra_headers or {})}
		...
		try:
			...
			if response.status_code == 401:
				raise CloudBrowserAuthError(
					'Authentication failed. Please make sure you have set browser_use_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.browser-agent.com/new-api-key'
				)
			elif response.status_code == 403:
				raise CloudBrowserAuthError('Access forbidden. Please check your browser-agent cloud subscription status.')
