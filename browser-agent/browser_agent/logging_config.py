...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	"""Setup logging configuration for browser-agent.

	Args:
		stream: Output stream for logs (default: sys.stdout). Can be sys.stderr for MCP mode.
		log_level: Override log level (default: uses CONFIG.BROWSER_AGENT_LOGGING_LEVEL)
	...
	log_type = log_level or CONFIG.BROWSER_AGENT_LOGGING_LEVEL
	...
	class BrowserUseFormatter(logging.Formatter):
		...
		def format(self, record):
			...
			if self.log_level > logging.DEBUG and isinstance(record.name, str) and record.name.startswith('browser_agent.'):
				...
				if 'Agent' in record.name:
					...
					record.name = 'dom'
				elif record.name.startswith('browser_agent.'):
					# For other browser_agent modules, use the last part
	...
	# Configure browser_agent logger
	browser_use_logger = logging.getLogger('browser_agent')
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
	...
	for name in ['browser_agent.agent', 'browser_agent.tools']:
	...
	for name in ['bubus', 'browser_agent.browser.session']:
		logger = logging.getLogger(name)
