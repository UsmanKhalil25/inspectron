...
- **Agent (`browser_agent/agent/service.py`)**: The main orchestrator that takes tasks, manages browser sessions, and executes LLM-driven action loops
- **BrowserSession (`browser_agent/browser/session.py`)**: Manages browser lifecycle, CDP connections, and coordinates multiple watchdog services through an event bus
- **Tools (`browser_agent/tools/service.py`)**: Action registry that maps LLM decisions to browser operations (click, type, scroll, etc.)
- **DomService (`browser_agent/dom/service.py`)**: Extracts and processes DOM content, handles element highlighting and accessibility tree generation
- **LLM Integration (`browser_agent/llm/`)**: Abstraction layer supporting OpenAI, Anthropic, Google, Groq, and other providers
...
Uses `cdp-use` (https://github.com/browser-agent/cdp-use) for typed CDP protocol access. All CDP client management lives in `browser_agent/browser/session.py`.
...
We use a thin wrapper around CDP called cdp-use: https://github.com/browser-agent/cdp-use. cdp-use only provides shallow typed interfaces for the websocket calls, all CDP client and session management + other CDP helpers still live in browser_agent/browser/session.py.
...
- **Browser Profile**: `browser_agent/browser/profile.py` contains all browser launch arguments, display configuration, and extension management
- **System Prompts**: Agent prompts are in markdown files: `browser_agent/agent/system_prompt*.md`
...
Connection management lives in `browser_agent/mcp/client.py`.
