...
import { CrawledPagesPanel } from "./components/crawled-pages-panel"
...
export default function CrawlerInterface() {
  ...
  const simulateCrawl = useCallback((inputUrl: string) => {
    ...
    const mockPages: CrawledPage[] = [
      { id: "1", url: inputUrl, title: "Homepage", status: "crawled", depth: 0, timestamp: new Date() },
      { id: "2", url: `${inputUrl}/about`, title: "About Us", status: "crawling", depth: 1, timestamp: new Date() },
      { id: "3", url: `${inputUrl}/products`, title: "Products", status: "pending", depth: 1, timestamp: new Date() },
      { id: "4", url: `${inputUrl}/blog`, title: "Blog", status: "pending", depth: 1, timestamp: new Date() },
      { id: "5", url: `${inputUrl}/contact`, title: "Contact", status: "pending", depth: 1, timestamp: new Date() },
    ]
    ...
    const interval = setInterval(() => {
      if (index < mockPages.length) {
        ...
        setCrawledPages((prev) => prev.map((page) => ({ ...page, status: "crawled" as const })))
  ...
  return (
    <div className="flex h-screen flex-col bg-background">
      <CrawlerHeader crawlStatus={crawlStatus} pagesCount={crawledPages.length} />
      ...
      <div className="flex-1 overflow-hidden">
        <div className="grid h-full grid-cols-1 lg:grid-cols-12 gap-0">
          ...
          <div className="lg:col-span-3 flex flex-col border-r border-border">
            ...
            />
            <CrawledPagesPanel pages={crawledPages} onPageSelect={handlePageSelect} selectedUrl={currentPreviewUrl} />
          </div>

          {/* Center Panel - Live Preview */}
          <div className="lg:col-span-6 border-r border-border">
            <LivePreviewPanel url={currentPreviewUrl} crawlStatus={crawlStatus} />
          </div>
